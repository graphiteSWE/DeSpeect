...
TEST(Command, createaudiosuccess){
    ...
    const std::list<std::string> processorName= commands->getUttProcessorsNames();
    for(std::list<std::string>::const_iterator it=processorName.begin(); it!=processorName.end(); it++) {
